...
async function main() {
  console.log('=== DUPLICATE BLOG POST CLEANUP ===\n');
  
  // Step 1: Fetch all English posts
  ...
  // Step 2: Fetch all French posts
  console.log('📥 Fetching French posts...');
  const frUrl = `${SUPABASE_URL}/rest/v1/blog_posts?language=eq.fr&status=eq.published&select=id,slug,title,created_at,updated_at&order=created_at.asc`;
  const frenchPosts = await makeRequest(frUrl);
  console.log(`✅ Found ${frenchPosts.length} French posts\n`);
  
  // Step 3: Fetch all Korean posts
  console.log('📥 Fetching Korean posts...');
  const koUrl = `${SUPABASE_URL}/rest/v1/blog_posts?language=eq.ko&status=eq.published&select=id,slug,title,created_at,updated_at&order=created_at.asc`;
  const koreanPosts = await makeRequest(koUrl);
  console.log(`✅ Found ${koreanPosts.length} Korean posts\n`);
  ...
  const frFinal = await makeRequest(`${SUPABASE_URL}/rest/v1/blog_posts?language=eq.fr&status=eq.published&select=id`);
  const koFinal = await makeRequest(`${SUPABASE_URL}/rest/v1/blog_posts?language=eq.ko&status=eq.published&select=id`);
