...
> **Status note (added 2026-08-13):** this is the initial research synthesis that informed the original mysterymaker.party SEO/GEO strategy — a hypothesis baseline, not a validated playbook updated from our own results. Where later ADRs/CHANGELOG entries (see `docs/adr/`, especially ADR-0045/0046) found a specific recommendation here wasn't working in practice, treat the newer, evidence-based finding as superseding this document — but expect that to be hard to detect in general, since most of what's below was never set up to be measured against outcomes in the first place. Don't treat this file as current gospel; treat it as where the strategy started.
