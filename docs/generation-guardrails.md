...
Canonical rule list for the **Make.com child scenario** per-character prompt (and the parent detective/evidence prompt where noted). Companion to `docs/evidence-card-generation-guardrail.md`. Source decisions: ADR-0037 (Whispers playtest), ADR-0041 (identity contamination), ADR-0042 (content-quality detectors + G7–G10), ADR-0080 (G12, real-world-sensitive harm themes). **Latest blueprint files on disk (re-verified 2026-08-13 by re-listing `temp-files/` — this line had gone stale before, see the ledger row below):** child `MM Live - Child (Unified)27-RealWorldHarmThemesGuardrail` (built 2026-08-13, carries G12 on top of Child26's G1/G2/G8/G9 — **imported to Make.com same day, live-confirm pending**), parent `MM Live - Parent53 (Real-World Harm Themes Guardrail)` (built 2026-08-13 on top of Parent52, carries G12 in both style routes' Part 1/Part 2 master_context prompts — **not yet imported**; Parent52 itself, from the blanket-Sonnet-5-upgrade line, supersedes Parent49, which this doc previously and incorrectly called current). **⚠️ Whether these are the versions actually LIVE in Make.com is not tracked in git — see the Import ledger at the bottom. Confirm the live version before assuming a guardrail is active. Re-list `temp-files/` yourself before building on top of either pointer — this file has already gone stale once.**
...
## G12 — No real-world-sensitive harm themes (sexual assault, trafficking, predation on young women) *(child content_coherence_rules + parent master_context, BOTH style routes)*

Character secrets, backstories, motives, and rumors must not center on sexual assault, human trafficking, or the sexual exploitation/predation of minors or young women — whether the character is the perpetrator, an enabler, or someone who witnessed such harm and stayed silent about it. This applies regardless of role (suspect, murderer, accomplice, victim). Other dark/adult genre themes (fraud, violence, addiction, affairs, blackmail over non-sexual misconduct) remain unrestricted — this is a narrow carve-out, not a general softening. (Incident: ADR-0080 — "The Host Herself" character Avery Blackwood's secret centered on witnessing and staying silent about a photographer preying on young models; customer flagged it given the current news climate. DB-wide search found no other package with this combination, but nothing in the prompt prevented it from recurring.)

**Status: not yet built into a blueprint file.** Prompt text is ready below; per the G7–G10/G11 convention, building it into the live blueprint is a separate step (duplicate the current head — re-list `temp-files/` first — insert, save as the next number, verify JSON valid + module count unchanged, import, record in the ledger below).

> Prompt line (child `content_coherence_rules`, parent master_context, mirrored in `regenerate-child-content` / `regenerate-parent-content`): *"REAL-WORLD-SENSITIVE HARM THEMES: Do not write any character's secret, backstory, motive, or rumor around sexual assault, human trafficking, or the sexual exploitation/predation of minors or young women — whether this character is the perpetrator, an enabler, or someone who witnessed such harm and stayed silent about it. This applies regardless of the character's role (suspect, murderer, accomplice, victim). Other dark or adult themes (fraud, violence, addiction, affairs, blackmail over non-sexual misconduct) remain in scope and do not need to be softened."*
...
| Child (Unified)26-DropSelfCheckTruncationFix — detective-style `finalStatement` accomplice-coherence carve-out + role guidance (ADR-0070) | 2026-08-11 (built on 25, module 409 "Call 3: Round Scripts" only) | ACCOMPLICE COHERENCE rule scoped to Rounds 2-4 with an explicit `finalStatement` carve-out; `finalStatement` schema field gives murderer/accomplice/suspect separate guidance | not directly confirmed at build time — **confirmed live 2026-08-12** (see below) | ✅ Live-confirmed via a real production generation, not just the 2026-08-11 disposable test: "Contamination: Death At The Infection Control Expo" (2026-08-12, customer `chelseagirrior@gmail.com`) — murderer and named accomplice both produced correct confession/admission pairs. Generic `**YOUR FINAL STATEMENT**` header for every role is still unfixed (cosmetic, noted in ADR-0070, not blocking). |
| G12 — No real-world-sensitive harm themes, child scenario (ADR-0080) | 2026-08-13 — built `temp-files/MM Live - Child (Unified)27-RealWorldHarmThemesGuardrail.blueprint.json`, duplicated off the TRUE current head (Child26 — confirmed by re-listing `temp-files/` first, per the blueprint-versioning hazard). Inserted into all 8 `content_coherence_rules` closing points via a node script (not sed/perl — shell-quoting broke on the apostrophes); JSON valid, module count unchanged (48), structural diff empty aside from the 8 new text insertions. | `REAL-WORLD-SENSITIVE HARM THEMES:` line in child `content_coherence_rules` | **2026-08-13 (Jonathan, Make UI)** | **Live-confirm pending** — imported same day as built; not yet verified against a real production generation. Grep the live prompt for the marker, or check the next character-heavy generation's `mystery_characters` rows, to close this out. **Recovery-tool mirror IS live and confirmed**: `regenerate-child-content` v3→v5 (redeployed twice — v4 accidentally shipped placeholder text instead of the real file content, caught immediately and corrected in v5; confirmed via `get_edge_function` that v5 serves the real code with the G12 line and zero placeholder residue). Incident fix (Avery Blackwood, "The Host Herself") applied by hand directly to `mystery_characters`; DB-wide sweep found no other package with this exact combination. |
| G12 — No real-world-sensitive harm themes, PARENT scenario (ADR-0080) | 2026-08-13 — built `temp-files/MM Live - Parent53 (Real-World Harm Themes Guardrail).blueprint.json`, duplicated off the TRUE current head (Parent52, confirmed by re-listing `temp-files/`). **Correction, same day:** initially judged the parent unnecessary based on the master_context *schema* (no prose field); pulling the actual `master_context` for the Avery Blackwood package proved that wrong — `howVictimWrongedThem`/`threatsOrPressures`/`specificMotive` (Part 1 + Part 2, both style routes) already contained the seed phrase the Child then elaborated into full prose. 4 insertion points (`## VICTIM PROFILE` in each style's Part 1, `## MOTIVE DISTRIBUTION` in each style's Part 2), inserted via a node script using `JSON.parse`/`JSON.stringify` (file is compact single-line JSON already, no formatting drift). JSON valid, module count unchanged (164), all 4 spot-checked in context. | `Content boundary:` line in the VICTIM PROFILE / MOTIVE DISTRIBUTION sections (a **content marker**, not a topology change) | **2026-08-13 (Jonathan, Make UI)** | **Live-confirm pending** — imported same day as built; not yet verified against a real production generation (needs a character-heavy paid mystery to run through both Parts and both style routes before this can be marked ✅). No edge-function mirror needed — `regenerate-parent-content` never regenerates `master_context` (see ADR-0080). |
| Parent blueprint pointer correction (found 2026-08-13, incidental to G12 work) | n/a | n/a | n/a | This doc's "Latest blueprint files on disk" line (top of file) named Parent49 as current — actually **stale**. `temp-files/` has Parent50 (Reveal-Confession Split, 2026-08-06), Parent51 (Blanket Sonnet Upgrade, 2026-08-11), and **Parent52 (Fix textResponse Field Mapping, 2026-08-11 23:27)** on top of it, none logged in this ledger. Parent52 is the true current head. G12 was NOT built into the parent (see ADR-0080 — scoped to the child, where the actual incident originated); whoever builds it next must duplicate Parent52, not Parent49. |
