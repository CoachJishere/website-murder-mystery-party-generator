...
- **Status:** Accepted, rotation complete — code fix shipped, URL rotated, Parent scenario patched (pending Jonathan's import); Make.com-side shared-secret check still an open recommendation
- **Date:** 2026-09-18
...
4. **Child scenario record.** Per this project's own blueprint-versioning discipline (never reuse a version number, always work from the latest export), Jonathan is exporting the post-rotation Child scenario as `temp-files/MM Live - Child (Unified)41-WebhookRotation.blueprint.json` so future Child-side changes build off the current live state rather than the stale v40 mirror.
...
- `temp-files/MM Live - Parent67 (Child Webhook URL Rotation).blueprint.json` — Parent blueprint with all 4 Child URL references updated; awaiting import
- `temp-files/MM Live - Child (Unified)41-WebhookRotation.blueprint.json` — post-rotation Child scenario export (pending from Jonathan)
