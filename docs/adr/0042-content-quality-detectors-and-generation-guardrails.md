...
## Update — 2026-07-26: G9 gap in the detective_script accomplice-beat → Parent49

Parent48 went live 2026-07-25. The next day a **G9 gap** surfaced in production: "The Black Swan Society" (a character/slip-style game) produced a no-accomplice `detective_script` that leaked the bracketed authoring note `[The master_context defines NO accomplice…]` to the customer — the same failure class G9 was meant to prevent (cf. Crimson Tide). The detector caught it pre-delivery and the package was fixed in place.

Root cause: G9's `<output_hygiene>` is a *general* rule ("never emit bracketed authoring directions"), but the detective_script reveal carries a *specific* ACCOMPLICE-BEAT instruction that ended "if there is NO accomplice, omit this beat entirely." That phrasing invited the model to acknowledge the omission inline rather than truly saying nothing, and the specific instruction won over the general rule.

Fix — **Parent49** (`temp-files/MM Live - Parent49 (Accomplice-Beat Silent-Omission).blueprint.json`, base Parent48, 164 modules, built 2026-07-26): duplicated the current head and programmatically replaced the accomplice-beat instruction in **all 4 detective_script generators** (module ids 5000/5004/5008/5012 — the predetermined *and* character-style routes; two textual variants existed on disk, "murderer/detective" and "culprit/investigator", both replaced). The new wording is **byte-identical** to the recovery-tool mirror already shipped in `regenerate-parent-content/index.ts` (v6, commit 5f82946): it instructs the model to write nothing when there is no accomplice — "no paragraph, no placeholder, no bracketed note, and no sentence explaining that there is no accomplice" — and never to output the instruction itself. Verified: JSON valid, module count unchanged (164), diff limited to the 4 beat blocks + the `bp.name` bump. **Not yet imported** (manual Make UI step); Parent48 stays live until then, and the recovery-tool mirror already protects ad-hoc regenerations. Ledger row added.

Lesson: a general hygiene rule does **not** neutralize a specific instruction that locally contradicts it — the specific instruction must itself be made hygienic. Any future "omit X if not applicable" instruction in a generator prompt should spell out *silent* omission, not just omission.
