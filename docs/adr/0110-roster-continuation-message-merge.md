...
*(The third occurrence did arrive — Addendum 2, same day as this note was superseded — and ADR-0125 did the consolidation: `mystery-webhook-trigger` and the preview page now both call one shared module.)*
...
*(Resolved same day by ADR-0125: the consolidation that followed this addendum moved `extractRosterFromMessage` — batch-path fix included — into a module `mystery-webhook-trigger` now imports directly, so this gap no longer exists on either side. Left as-written above since it was accurate at the time; see ADR-0125 for the actual fix.)*
...
- `src/pages/MysteryPurchase.tsx` — `isGroupHeaderLine`, wired into `scanWholeMessageForRoster` and `scanForRoster` (this whole file's roster parser was itself deleted later the same day by ADR-0125 — see that ADR)
- `supabase/functions/mystery-webhook-trigger/index.ts` — at the time of this addendum, `extractRosterFromMessage`'s batch-path fallback had the same unpatched gap (see "Not addressed" above); resolved same day by ADR-0125's consolidation
