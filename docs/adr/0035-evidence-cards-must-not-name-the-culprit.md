...
## Decision
1. **Contract:** the player-facing evidence DESCRIPTION must never name (or uniquely pinpoint) the culprit or accomplice. It may carry *deductive breadcrumbs* — initials, matching handwriting, a timeline gap, an unnamed "one suspect" — but the name lives only in the host-only SIGNIFICANCE.
   - **Amendment (2026-07-20) — clue timing & strength (the funnel):** breadcrumbs must also respect *when* and *how strong*. Round 2 = motive (broad, 4–6 suspects, **no identity-narrowing clues at all** — no initials/monograms/signatures/one-person handwriting); Round 3 = method/means (narrowing); Round 4 = opportunity (the subtle flaw). Identity breadcrumbs live only in Rounds 3–4, each one link in a chain. A breadcrumb that resolves **1:1** to a single cast member (e.g. initials matching exactly one character) is *strong* — at most one, in Round 4, never in Round 2.
...
**Follow-up from playtest (2026-07-20):** keeping "S.C." in the *round-2* journal turned out to be a bit too obvious — a careful player could shortcut to the culprit on the first evidence card, before Rounds 3–4 did any narrowing (it went unnoticed live only because the host, playing the detective, didn't linger on it). This exposed the missing dimension: the original contract governed *whether* a card names the culprit but not *when* an identity clue may appear or *how strong* it may be. An initial that resolves 1:1 to a single cast member is nearly a name; placed in the motive round it collapses the funnel. Hence the clue-timing amendment above: motive round stays identity-free, identity breadcrumbs move to Rounds 3–4, and 1:1 clues are Round-4-only. The *Whispers* round-2 journal is corrected to "a second, hurried hand" (no initials) — the Round-3 handwriting-match still carries the chain.
