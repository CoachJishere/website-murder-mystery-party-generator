...
## Addendum (2026-09-11): root cause found — oversized `conversationContent` silently no-ops the scenario

This ADR's own Consequences section named "why rows go missing in the first place" as not addressed. This incident found (at least) one real cause, and it also implicated the recovery mechanism itself, not just the original generation.

**Incident:** package `732145f5-133c-4c6f-a8d3-1795c55353b5` ("Glamour, Gier Und Ein Glas Champagner", conversation `fc756bcc-b56e-428b-8f47-bb2b73a97493`, customer Emma Schulz) shipped with 3 of 8 characters (Abby Sterling, Victoria von Hohenau, Veronika Schöffner) never created. `sweep_stuck_needs_review_packages` (every 10 min) kept calling this function, which kept firing `CHILD_WEBHOOK` for the 3 missing names — that repeated firing is what Jonathan noticed costing him money in Make.com, separate from any Anthropic spend.

**Root cause:** `conversationContent` (line ~478, before this addendum) sent `pkg.user_conversation` **raw and whole**. This customer's transcript was ~180,000 characters — unusually long. Two automatic attempts (capped by `MAX_ATTEMPTS_PER_CHARACTER`, exactly as designed) both "succeeded" at the Make.com level (HTTP 200 `Accepted`, scenario execution status `1`) while writing nothing. Confirmed via the Make.com API (`GET /scenarios/{id}/logs`): a real character generation is **19 operations / ~100s**; these failed runs were **7 operations / ~47s** — fewer steps, and a large `transfer` size (the oversized payload still had to go over the wire) — consistent with an early branch/error-handler inside the scenario swallowing something (most likely a Claude call rejecting the request as too long) instead of surfacing a hard failure. A same-shaped manual retry (identical payload) failed the same way a 3rd time, ruling out a transient flake.

**Fix applied:** a same-day manual retry using a **name-windowed excerpt** of the same transcript (~15,000 chars: opening context + ±1,200 chars around every mention of the target character's name) succeeded on the first attempt for all 3 characters. `buildConversationExcerpt()` was added to `notify-generation-issue/index.ts` (deployed as function version 33) to apply this windowing permanently to every re-fire, not just this one — see the function's own doc comment for the mechanics and the size constants.

**Deploy note:** the first deploy attempt of this fix (function version 32) accidentally shipped literal placeholder text as the entire function body instead of the real file — a copy-paste error in the deploy call, not a code bug. Caught within ~1-2 minutes by re-fetching the deployed source and diffing it against the intended change; verified via `net._http_response` that zero real invocations hit the broken version before it was corrected (version 33). No customer impact, but worth naming here as a reminder to verify a deploy's actual shipped content immediately after any edge function deploy, not just its HTTP status.

**Policy decision — keep the 2-attempt-then-pause cap.** Jonathan raised the question directly: given the automatic recovery didn't actually succeed on its own here (it needed manual intervention), was the money spent on 2 automatic attempts per character well spent, or should the cap change? Conclusion: **keep `MAX_ATTEMPTS_PER_CHARACTER = 2` as-is.** The cap did exactly its job — it stopped the loop and bounded spend without anyone needing to intervene to *stop* it (the only intervention needed was to *fix* it, a different problem). Raising the attempt count would not have helped: the failure was deterministic given the oversized payload, not a coin-flip that more tries would eventually win. The real lever was fixing the payload size, which is now done — a future oversized-transcript case should now just succeed on attempt 1 instead of needing a human.

**Not addressed by this addendum:** whether other re-fire-shaped code paths (e.g. `regenerate-child-content`'s own `conversationContent: pkg.user_conversation ?? ""` reference, noted in the pre-existing comment this addendum's code sits next to) have the same oversized-payload exposure. Worth a follow-up grep (`grep -rn "user_conversation ?? \"\"" supabase/functions/`) next time someone is in this area, not urgent enough to block this fix.

### Key files (addendum)
- `supabase/functions/notify-generation-issue/index.ts` — added `buildConversationExcerpt()`, wired into the `CHILD_WEBHOOK` payload (function version 33)
