...
## Addendum 2 (2026-09-19): Same fix extended to the other 5 auto-remediated detectors

Checking whether Addendum 1's bug had siblings: `auto-remediate-packages`' `DETECTOR_RPC` map shows 5 more health-check detectors are self-healed by the exact same 30-minute cron — `list_packages_with_identity_conflicts` (check 5), `list_packages_with_meta_text_leak` (check 6), `list_packages_with_victim_mismatch` (check 7), `list_packages_with_slip_culprit_leak` (check 8), and `list_packages_with_self_directed_questions` (check 9). All 5 had the identical gap: a 30-day `_since` window that only ages out *old* incidents, with zero protection against a package that completed minutes ago and hasn't reached the next remediation pass. Same false-alarm shape as Addendum 1, just waiting to happen on a different defect class next time.

**Fix:** added the same 45-minute `generation_completed_at` grace period to all 5 (migration `20260919173000_grace_period_content_quality_detectors.sql`), scoped to the `'completed'` branch only. Deliberately left the `'needs_review'` branch of these queries untouched: `generation_completed_at` isn't a meaningful "how long has this defect existed" signal for a package that was demoted rather than cleanly completed, that state already has its own established handling (check 3's 10-minute `needs_review_at` hold), and a package sitting in `needs_review` is already the more urgent case these checks exist to catch — not the transient one this addendum narrows.

Pulled each function's live definition via `pg_get_functiondef` immediately before writing the migration (rather than reconstructing from migration files) to guarantee the patch applied to the actual current body, not a possibly-stale historical copy — these 5 functions had each been through 2-3 rounds of `CREATE OR REPLACE` across different migration files since original creation. Applied directly to the linked project via `supabase db query --linked -f`; verified no regression by re-running all 5 detectors with the same 30-day `_since` the health check actually passes and confirming the counts match the last real health-check run's committed `monitoring/health-status.md` (all zero, aside from the now-fixed evidence-images check) — the one pre-existing non-zero result (`identity_conflicts` = 1, the known "Bollinger" incident from 2026-07-02, already excluded by the workflow's own jq filter) is from July and outside both the 30-day window and the 45-minute grace period either way, so unaffected by this change.
