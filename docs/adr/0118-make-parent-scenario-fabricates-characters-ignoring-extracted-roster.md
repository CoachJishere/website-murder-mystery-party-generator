...
## Addendum 2 (2026-09-04, same day): built the "character absent from conversation" detector, swept all paid packages, one real (unrelated) finding
...
**The remaining flag is a real, different, minor bug — not a duplicate of this incident.** "Tod Auf Der Alm 3000" (`d5d66a5b-...`, already tracked for the ADR-0103 Addendum 13 Sie/du register issue) has a character row with `character_name = 'Marina'`, but that row's own `background` field introduces them as "**Name:** Captain Robin/Robina Frost" — the outward name column and the authored identity inside the character's own content disagree. This is a name-drift/staleness bug (the content was written for one identity while the name column reflects an earlier or different one), not a fabricated-content bug — the content itself isn't invented, it's just filed under the wrong name. Reported to Jonathan for a decision on whether to fix now or track separately; not fixed as part of this addendum.
