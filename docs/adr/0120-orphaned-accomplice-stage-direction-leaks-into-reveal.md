...
**Verified the 62 were actually affected before touching anything.** A first pass matching the literal English bracket `*[The murderer/culprit (Name/player) reads their confession aloud.]*` hit 59/62. Manually read the 3 remaining: two were genuinely non-English despite `profiles.language` saying "en" for all three (Dutch, Latvian — confirms `profiles.language` is not a reliable signal for a package's actual generated-content language, consistent with prior language-tracking bugs in this project's history), the third was Spanish with a translated section heading my first regex's `## THE REVEAL` anchor also missed. All 3 turned out to carry the identical defect in their own language — 62/62, not an estimate with a long tail of exceptions. **Not a new finding — already fully documented by ADR-0112**, which established that `profiles.language` is an account-level UI/email-locale flag (set from whatever language the customer is currently browsing the site in), deliberately independent of the language a given conversation's content is written/generated in; the real generation-language signal since 2026-08-29 is `detectConversationLanguage()`'s Haiku classifier reading the conversation text itself. All 3 mismatched packages here predate that fix (Aug 2, Aug 22, Aug 28) but their actual delivered content was already correctly in-language when read — so this is the expected divergence ADR-0112 describes, not a content-generation bug. Flagging here only as a process note: don't use `profiles.language` as a language signal in future corpus-wide sweeps — read the content itself instead.
