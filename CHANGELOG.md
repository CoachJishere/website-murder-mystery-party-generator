...
Re-derived from `npm audit` against the committed `package-lock.json` (the repo's only lockfile, so it's what Dependabot scans). The circulating **"10 vulns / 6 high" figure was stale** — actual state was **7 root advisories (4 high, 2 moderate, 1 low)**. Triage weighted real exploitability for a static React/Vite SPA on GitHub Pages with no server of its own (Supabase is a separate backend), rather than chasing the count to zero.
