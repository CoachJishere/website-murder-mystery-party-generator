...
- **Entry gate (prevention)** — [mystery-webhook-trigger/index.ts](supabase/functions/mystery-webhook-trigger/index.ts) now refuses to fire Make.com when the conversation has no assistant "CHARACTER LIST" message and no approved concept (the `characterListRegex` is hoisted so the gate and the concept-snapshot share one definition). It writes `needs_more_info` and returns `200 {success:false, reason:'needs_more_info'}` (service/recovery callers bypass). ⚠️ **Requires an edge-function deploy to go live.**
