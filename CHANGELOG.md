...
- **Schema** (auto-generated from content patterns in [BlogPost.tsx](src/pages/BlogPost.tsx), no code change): the new FAQ (9 Q&As incl. "How to run an office murder mystery party?") drives **FAQPage**, and the `## How to` + `## Step N:` headings now trigger **HowTo** (5 steps) — verified by replicating the detector regexes before ship. Detector for HowTo is English-keyword based, so it fires for EN only (see deferred note).
