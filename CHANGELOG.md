...
### Feature: single source of truth for the "your mystery is ready" email, both halves shipped and verified end-to-end ([ADR-0106](docs/adr/0106-single-source-of-truth-ready-notification.md))
Follow-up to today's duplicate-email finding (Toby Wragg's purchase, ADR-0104 Addendum 2): the "Your Mystery is Ready! We'd Love Your Feedback" email lived as 12 duplicated HTTP modules inside the Parent Make.com blueprint, each behind its own "has empty characters → retry → recheck" sub-flow that raced against the separate, more careful `notify-generation-issue` recovery system without either knowing the other existed — causing the duplicate send and roughly doubling generation time/spend on any package that hit an initially-empty character. DB-side half: new `ready_email_sent_at` column on `mystery_packages`, a `BEFORE INSERT OR UPDATE` trigger (`notify_package_ready()`) that fires exactly once on the genuine transition into `generation_status.status = 'completed'` regardless of which code path caused it, and a new `send-mystery-ready-email` edge function it calls. Parent blueprint half: removed the 4 retry routers and all 12 embedded email modules, published as `Parent57` — which shipped with a real regression (caught in the same session, before any real customer hit it): those routers' "no empty characters" exit also contained the *only* writes that ever marked a package `completed`, so Parent57 would have generated content correctly but never marked anything done. Caught via the planned post-deploy test run itself (a 4-character test package stalled at 90%, "Verifying character content...", generation_completed_at never set, even though Make's own logs showed the Parent and all 4 Child runs had finished). Fixed and republished as `Parent58`: reuses the original routers' own filter conditions verbatim but with only the completion-marking writes restored, no retry, no email. Also fixed a second, previously-unnoticed bug found while reading the original blueprint: the "still broken after retry" exit used to send the customer the ready email and mark the conversation complete even when generation was confirmed still broken — Parent58's `needs_review` branch does neither. Verified fully end-to-end with real spend: a fresh 4-character test package hit a genuine defect (one character's round content blocked by Anthropic's content filter, same failure class as Sylvie/Sylvain), correctly routed to `needs_review` by Parent58, recovered by `notify-generation-issue`, promoted to `completed` by the existing sweep cron, and the new trigger fired exactly one "ready" email (confirmed via `net._http_response`: one 200, no duplicates).

**Follow-up, same day (Parent60):** a second real test found Parent58's completeness check firing before Child had actually finished — `145` (fires the Child webhook) is fire-and-forget, and `4001` (labeled "Wait for all child fires" in the designer) only aggregates the firing loop's own metadata, not any real completion signal. Confirmed directly: `generation_completed_at` was set at `21:09:29` while Child scenario executions were still running until `21:09:35`/`21:10:12`. All 4 characters got flagged and re-fired; one (Absalom Reyes) turned out to have been fine the whole time — his content just hadn't landed yet, an avoidable `$0.15` recovery cost. The original retry loop's 5-minute sleep used to incidentally absorb this same race; removing the retry loop removed that buffer too. Fixed: added one 90-second `util:FunctionSleep` per branch, once, positioned after Parent's own work and immediately before the completeness check — not tied to any loop or retry. Published as `Parent60`, built on `Parent59` (a different, unrelated same-day fix from another concurrent session — the victim dual-name issue below). Verified structurally (100 modules, all 4 sleeps confirmed in position, zero dangling references) before publishing; not yet re-verified with a live run.
