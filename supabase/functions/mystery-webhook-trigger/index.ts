...
function extractCharactersFromMessages(rawMessages: any[], approvedMessageId?: string | null, playerCount?: number | null): ExtractedCharacter[] | null {
  ...
  // ADR-0125: this used to re-parse latestMessageWithList itself via two more
  // independent, inline-duplicated regex passes ("Primary" header-scan, then a
  // "Secondary" 4+-consecutive-bold-line batch-scan across every assistant
  // message) - a THIRD copy of the exact logic `extractRosterFromMessage`
  // already implements, discovered while consolidating the client/server split
  // that ADR-0110 Addenda 1-2 kept hitting. It was pure redundancy:
  // `findLatestConceptMessage` above only ever returns a message for which
  // `extractRosterFromMessage` already yields >= MIN_ROSTER_SIZE, so re-deriving
  // that same roster by hand here could only ever reproduce it - or, since this
  // copy's batch-scan carried the same unpatched small-sub-group-drops-below-4
  // bug ADR-0110 Addendum 2 fixed in the shared module, occasionally reproduce
  ...
  console.log(`[CharExtract] No plausible roster found via regex extraction`);
  return null;
}
