...
console.log('🔄 UPDATING 3 Optimized Posts (Zombie/Superhero/Medieval)\n');
...
for (const post of posts) {
  console.log(`\n🔍 Finding existing post: ${post.title.substring(0, 50)}...`);
  ...
  if (findError || !existing) {
    console.log(`  ❌ Not found in database - ${findError?.message || 'no match'}`);
    results.push({ title: post.title, success: false, error: 'Not found' });
    continue;
  }

  console.log(`  ✅ Found! ID: ${existing.id}, Status: ${existing.status}`);
  console.log(`  📝 Updating with optimized content...`);
  ...
  if (updateError) {
    ...
    results.push({ title: post.title, success: false, error: updateError.message });
  } else {
    console.log(`  ✅ Successfully updated and published!`);
    results.push({ title: post.title, success: true, id: updated[0].id });
  }
...
console.log(`✅ Successfully updated: ${successful}/3 posts`);
if (failed > 0) {
...
if (allEnglish.length === 61) {
  console.log('\n🎉 SUCCESS! All 61 posts from your original list are now published and optimized!');
} else if (allEnglish.length === 58) {
  console.log('\n⚠️  Still at 58/61 - the 3 updates may have failed.');
} else {
