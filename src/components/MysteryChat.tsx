...
export default function MysteryChat({
  ...
  useEffect(() => {
    if (needsInitialAIResponse && !hasTriggeredInitialResponse && messages.length > 0 && !isLoadingHistory) {
      // Don't trigger if the last message is already from the AI
      const lastMessage = messages[messages.length - 1];
      if (lastMessage?.is_ai) {
        console.log("Skipping initial AI response — last message is already from AI");
        return;
      }
...
`;
  ...
  const callAIWithRetry = async (messages: any[], systemPrompt: string | null, retryCount = 0, language?: string): Promise<any> => {
    ...
    try {
      ...
      if (error) {
        // The Edge Function's actual error body (e.g. { error, code }) lives on
        // error.context (the raw Response) — error.message from supabase-js is
        // just a generic "non-2xx status code" string. Read the body so callers
        // can tell a deterministic validation failure (message too long) apart
        ...
        if (errorCode) (wrapped as any).code = errorCode;
        throw wrapped;
      }
      ...
      if (data && data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content) {
        ...
        if (data.code) (wrapped as any).code = data.code;
        throw wrapped;
      } else {
      ...
      // MESSAGE_TOO_LONG is a deterministic validation failure — the same
      // oversized message will be rejected identically on every retry, so
      // retrying just delays telling the user for no benefit.
      if ((error as any)?.code === 'MESSAGE_TOO_LONG') {
        throw error;
      }
  ...
  // Turns a failed AI call into a persisted chat bubble instead of a toast
  // that vanishes with no trace in the transcript — a customer whose message
  // got rejected (e.g. too long) would otherwise see the conversation just
  // go silent with nothing explaining why (ADR-0103, 2026-09-13).
  const appendErrorMessage = async (error: unknown) => {
    const content = (error as any)?.code === 'MESSAGE_TOO_LONG'
      ? (error as Error).message
      : t("chat.errors.aiUnavailable");
  ...
  const triggerAIResponse = async (userContent: string) => {
    // Safety guard: don't send a request if the last message is already from the AI
    const lastMsg = messages[messages.length - 1];
    if (lastMsg?.is_ai) {
      console.log("Aborting triggerAIResponse — last message is already from AI");
      setIsAiTyping(false);
      return;
    }
                ...
                <div className={cn(
                  ...
                  <ReactMarkdown 
                    ...
                    components={{
                      ...
                      )} style={{ fontFamily: 'var(--font-display)', color: '#E53E2A' }} {...props} />,
